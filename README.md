...
catalog membership, and the SHA-256 of the frozen `encore-app-architecture` and
`security-data-invariants` app-invariant specs. `npm run lockstep` and the `ci-lockstep` job fail visibly on any upstream
